	...
	bool OnUserUpdate(float fElapsedTime) override
	{
                ...
                if (iterations % slowdownAmnt == 0)
                {
                    for (int step = 0; step < 16; step++)
                    {
                        ...
                        int microcodeLocation = BinToDec(DecToBinFilled(InstructionReg, 16).substr(0, 4) + DecToBinFilled(step, 4) + flags[0] + flags[1]);
                        ...
                        { // ST
                            //Console.Write("ST ");
                            cout<<("\n== PAUSED from HLT ==\n\n");
                            cout<<("FINAL VALUES |=  o: " + outputReg + " A: " + AReg + " B: " + BReg + " bus: " + bus + " Ins: " + InstructionReg + " img:(" + imgX + ", " + imgY + ")\n");
                            system("pause");
                        }
...
for(int i=input.length()-1;i>=0;i--){
    ...
    string HexToBin(string hex, int desiredSize)
    {
        ...
        // Convert dec to bin
        std::string output = DecToBin(hex);
...
vector<string> explode(const string& str, const char& ch) {
    ...
    vector<string> parseCode(string input)
    {
        ...
        for (int i = 0; i < splitcode.Length; i++)
        {
            ...
            {
                cout<<(DecToHexFilled(Int32.Parse(splitBySpace[1]), 3));
    ...
    static void GenerateMicrocode()
    {
        ...
        for (int osind = 0; osind < sizeof(output)/sizeof(output[0]); osind++) { output[osind] = "00000"; }
        ...
        for (int ins = 0; ins < instructioncodes.Length; ins++) // Iterate through all definitions of instructions
        {
            ...
            for (int step = 0; step < instSteps.Length; step++) // Iterate through every step
            {
                ...
                for (int flagcombinations = 0; flagcombinations < flags.Length * flags.Length; flagcombinations++)
                {
                    ...
                    char[] newendaddress = DecToBinFilled(flagcombinations, 2).c_str();
        ...
        for (int ins = 1; ins < instructioncodes.Length; ins++) // Iterate through all definitions of instructions
        {
            ...
            for (int step = 0; step < instSteps.Length; step++) // Iterate through every step
            {
                ...
                for (int flagcombinations = 0; flagcombinations < flags.Length * flags.Length; flagcombinations++)
                {
                    ...
                    cout<<("\t& " + startaddress + " " + midaddress + " " + new string(newendaddress) + "  =  " + BinToHexFilled(stepComputedInstruction, 5));
                    cout<<endl;
                    output[BinToDec(startaddress + midaddress + new string(newendaddress))] = BinToHexFilled(stepComputedInstruction, 5);
                }
            }
